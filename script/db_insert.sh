...
options=("${RED}New movie collection${NC}" 
         "${YLW}New movie seen${NC}" 
         "${GRN}Create DB${NC}" 
         "Quit")
function mainmenu {
#Force list to be only one column
    COLUMNS=1
    echo "================================="
...
#Create insert_movie.csv
            ...
            echo "Created '${RED}insert_movie_${movie_id}.csv${NC}'."
        else
...
#Create insert_seen.csv
            ...
            echo "Created '${YLW}insert_seen_${movie_id2}.csv${NC}'."
        else
#If database doesn't exist alert the user
            echo -e "The database '${YLW}$DBNAME${NC}' doesn't exists."
        fi
    ;;
#If database doesn't exist create DB
    "${GRN}Create DB${NC}")
            sh ${SCRPATH}db_create.sh
    ;;
#Quit option
    "Quit")
            break
